...
void f3d_gyro_interface_init() {
  ...
  //intializing clock for A
  RCC_AHBPeriphClockCmd(RCC_AHBPeriph_GPIOA, ENABLE);

  //initialzing SCK(PA5)| MISO(PA6)| MOSI(PA7)
  GPIO_InitTypeDef GPIO_InitStructure_AF;
  GPIO_InitStructure_AF.GPIO_Pin = GPIO_Pin_5 | GPIO_Pin_6 | GPIO_Pin_7;
  ...
  GPIO_Init(GPIOA, &GPIO_InitStructure_AF);
  ...
  //initializing clock for E
  RCC_AHBPeriphClockCmd(RCC_AHBPeriph_GPIOE, ENABLE);
  
  // initialize CS(PE3)
  GPIO_InitTypeDef GPIO_InitStructure_Gyro;
  GPIO_InitStructure_Gyro.GPIO_Pin = GPIO_Pin_3;
  GPIO_InitStructure_Gyro.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_InitStructure_Gyro.GPIO_Mode = GPIO_Mode_OUT;
  GPIO_InitStructure_Gyro.GPIO_OType = GPIO_OType_PP;
  GPIO_InitStructure_Gyro.GPIO_PuPd = GPIO_PuPd_NOPULL;
  
  GPIO_Init(GPIOE, &GPIO_InitStructure_Gyro);
...
void f3d_gyro_write(uint8_t* pBuffer, uint8_t WriteAddr, uint16_t NumByteToWrite) {
  ...
  if(NumByteToWrite > 1){ 
    WriteAddr |= (uint8_t)(0x40); //multiple bit mode
  } 
  //SET THE CS
  GYRO_CS_LOW();
  //SEND THE FIRST BYTE
