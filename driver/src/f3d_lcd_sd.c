...
void f3d_lcd_sd_interface_init(void) {
 /* vvvvvvvvvvv pin initialization for the LCD goes here vvvvvvvvvv*/ 
  ...
  //initialize alternate pins
  GPIO_InitTypeDef GPIO_InitStructure_AF;
  GPIO_InitStructure_AF.GPIO_Pin = GPIO_Pin_13 | GPIO_Pin_14 | GPIO_Pin_15;
  ...
  GPIO_InitStructure_AF.GPIO_PuPd = GPIO_PuPd_NOPULL;
...
void f3d_lcd_fillScreenX(uint16_t color, int dgs) {
  uint8_t x,y;
  if(dgs >= 0){ //positive degrees
    dgs = scaler(dgs); //simple scaling function
    f3d_lcd_setAddrWindow(0, 80-dgs, 42, 80, MADCTLGRAPHICS);
    for (x=0; x < 42; x++){
      for(y=0; y < 80 - dgs; y++){
	f3d_lcd_pushColor(&color, 1);
      }
      ...
      dgs = scaler(abs(dgs)); //simple scaling function
      f3d_lcd_setAddrWindow(0, 80, 42, 80 + dgs, MADCTLGRAPHICS); 
      for(x=0; x < 42; x++){
	for(y=0; y < dgs; y++){
	  f3d_lcd_pushColor(&color, 1);
	}   
...
void f3d_lcd_fillScreenX_Nun(uint16_t color, int dgs){
  
  //initialize stuff
  uint8_t x,y;
  int restingValue = 505; // this represents the resting value of the nunchuk's x axis
 
  //put an upper and lower bound on x-rotations for simpler scaling
  if(dgs > 700){ //set upper bound
    dgs = 700;
  }
  if(dgs < 300){ //set lower bound
    dgs = 300;
  }
  
  //screen range for this display: 30-90-150
  
  if(dgs >= restingValue){// draw top to mid
    dgs = scaler_nun(abs(dgs - restingValue), 0);
    int y0 = 90 - dgs;
    int y1 = y0 + 10;
    f3d_lcd_setAddrWindow(0, y0, 42, y1, MADCTLGRAPHICS);
    for(x=0; x < 42; x++){
      for(y=0; y < 10; y++){
	f3d_lcd_pushColor(&color, 1);	
      }
    }
  } else { //draw mid to bottom
    dgs = scaler_nun(abs(dgs - restingValue), 0);
    int y0 = 90 + dgs;
    int y1 = y0 + 10;
    f3d_lcd_setAddrWindow(0, y0, 42, y1, MADCTLGRAPHICS);
...
void f3d_lcd_fillScreenY(uint16_t color, int dgs) {
  uint8_t x,y;
  //update
  if(dgs >= 0){ //positive degrees
    dgs = scaler(dgs); //simple scaling function
    f3d_lcd_setAddrWindow(42, 80- dgs, 84, 80, MADCTLGRAPHICS);
    for (x=42; x < 84; x++){
      for(y=0; y < 80 - dgs; y++){
	f3d_lcd_pushColor(&color, 1);
      }
      ...
      dgs = scaler(abs(dgs)); //simple scaling function
      f3d_lcd_setAddrWindow(42, 80, 84, 80 + dgs, MADCTLGRAPHICS); 
      for(x=42; x < 84; x++){
	for(y=0; y < dgs; y++){
	  f3d_lcd_pushColor(&color, 1);
	}   
...
void f3d_lcd_fillScreenY_Nun(uint16_t color, int dgs){
  
  //initialize stuff
  uint8_t x,y;
  int restingValue = 515;
  ...
  if(dgs <= restingValue){// draw top to mid
    ...
    int y0 = 90 - dgs;
    int y1 = y0 + 10;
    f3d_lcd_setAddrWindow(42, y0, 84, y1, MADCTLGRAPHICS);
    for(x=42; x < 84; x++){
      for(y=0; y < 10; y++){
	f3d_lcd_pushColor(&color, 1);	
      }
...
void f3d_lcd_fillScreenZ(uint16_t color, int dgs) {
  uint8_t x,y;
  if(dgs >= 0){ //positive degrees
    dgs = scaler(dgs); //simple scaling function
    f3d_lcd_setAddrWindow(84, 80-dgs, ST7735_width-1, 80, MADCTLGRAPHICS); 
    for (x=84; x < ST7735_width-1; x++){
      for(y=0; y < 80 - dgs; y++){
	f3d_lcd_pushColor(&color, 1);
      }
    }
      ...
      dgs = scaler(abs(dgs)); //simple scaling function
      f3d_lcd_setAddrWindow(84, 80, ST7735_width-1, 80 + dgs, MADCTLGRAPHICS); 
      for(x=84; x < ST7735_width-1; x++){
	for(y=0; y < dgs; y++){
	  f3d_lcd_pushColor(&color, 1);
	}   
...
void f3d_lcd_fillScreenZ_Nun(uint16_t color, int dgs){
  
  //initialize stuff
  uint8_t x,y;
  int restingValue = 635;
  
  //put an upper and lower bound on x-rotations for simpler scaling
  if(dgs > 735){ //set upper bound
    dgs = 535;
  }
  if(dgs < 535){ //set lower bound
    dgs = 535;
  }
  ...
  if(dgs >= restingValue){// draw top to mid
    dgs = scaler_nun(abs(dgs - restingValue), 1);
    int y0 = 90 - dgs;
    int y1 = y0 + 10;
    f3d_lcd_setAddrWindow(84, y0, ST7735_width-1, y1, MADCTLGRAPHICS);
    for(x=84; x < ST7735_width-1; x++){
      for(y=0; y < 10; y++){
	f3d_lcd_pushColor(&color, 1);	
      }
...
//scaler for board gyro data
int scaler(int y){ //this was supposed to be fancier
  return (int)(y/(7.5));
}

//scaler for nunchuk accel data
int scaler_nun(int y, int ax){ //this also was supposed to be fancier
  if(ax){
    return (int)y/(1.66); // for z: 100/60
  }else{
    return (int)(y/(3.4)); // for x, y: 200/60
  }
}
...
/***************************LAB 7 ADDITIONS BELOW**************************/
//draws arrow to lcd; intended to point north at all times
//pass in heading(yaw) as dgs so arrow can adjust accordingly
void f3d_lcd_drawCompass(int yaw, int overwrite){ //center on board is (64, 80): this will be our pivot
  char *c;
  if(yaw >= 0){ //handle positive degress
    if(yaw <= 15){
      c = "S";
    } else if(yaw <= 67) {
      c = "SE";
  ...
  if(overwrite){ //overwriting last compass heading 
    f3d_lcd_drawString(64, 80, c, WHITE, WHITE);
  } else {
    f3d_lcd_drawString(64, 80, c, BLACK, WHITE);
  }
}


/**************************LAB 7 ADDITIONS ABOVE***************************/
